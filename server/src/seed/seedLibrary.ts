...
export function seedLibrary(ctx: any): void {
  ...
  if (!ctx.db.Entity.id.find(kojiEntityId)) {
    ...
    ctx.db.Process.insert({ entityId: kojiEntityId, processKind: { tag: 'koji' }, notes: undefined });
    ...
    const kojiSteps: SeedStep[] = [
      ...
      { label: 'Shimai-shigoto / Final Work', atH: 38, durationH: undefined, goals: ['Ideal timing is around 38.5°C koji temperature', 'Koji temp target at start: 38–39°C', 'Chamber target: 35–36°C'], checks: ['Once koji reaches 40°C, or at shimai, remove lid and cover tub with a clean dry cotton towel'], actions: ['Mix thoroughly so grains are separated, record weight, then transition from lid to dry towel cover'] },
      { label: 'Peak Temperature Hold', atH: 42, durationH: undefined, goals: ['Target peak koji temperature: 40–41°C for a typical batch', 'Acceptable overall peak range: 40–43°C depending on strain and goals', 'Maintain near peak until de-koji'], checks: ['Start de-koji timing once peak temperature is reached', 'If temperature overshoots, mix and/or lower chamber by 1–2°C', 'Do not remove towel during correction or rice may dry too quickly'], actions: ['Hold near peak temperature and monitor for stability'] },
